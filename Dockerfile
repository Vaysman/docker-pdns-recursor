...
RUN sed -i 's/quiet=yes/quiet=no/' /etc/powerdns/recursor.conf && \
    ...
    sed -i 's/local-address=127.0.0.1/local-address=0.0.0.0/' /etc/powerdns/recursor.conf && \
...
RUN echo 'exec /sbin/setuser root /usr/sbin/pdns_recursor >>/var/log/pdns_recursor.log 2>&1' >> /etc/service/pdns-recursor/run
